...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const RAPIDAPI_KEY = Deno.env.get('ETSY_RAPIDAPI_KEY');
    
    if (!RAPIDAPI_KEY) {
      throw new Error('ETSY_RAPIDAPI_KEY environment variable is not set');
    }
    ...
    const { data: limitCheck, error: limitCheckError } = await supabase.rpc(
      ...
      { api_name: 'etsy', user_id_param: user.id }
    );
    ...
    if (!limitCheck) {
      throw new Error('You have exceeded your Etsy API usage limit for this month');
    }
    ...
    const apiUrl = `${BASE_URL}/product/search?${searchParams.toString()}`;
