  ...
  async searchProducts(params: EtsySearchParams): Promise<EtsyProduct[]> {
    ...
    return this.queueRequest(async () => {
      return this.retryRequest(async () => {
        console.log('🔍 Searching Etsy via Edge Function:', searchParams.query);
        ...
        const response = await fetch(
          `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/etsy-search?query=${encodeURIComponent(searchParams.query)}&page=${searchParams.page}`,
          {
            headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || `Etsy search failed with status ${response.status}`);
        }
