    ...
    try {
      ...
      return await this.queueRequest(async () => {
        return this.retryRequest(async () => {
          console.log('Searching Amazon via Edge Function:', searchParams.query);
          ...
          // Call the Supabase Edge Function instead of RapidAPI directly
          const response = await fetch(
            `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/amazon-search?query=${encodeURIComponent(searchParams.query)}&page=${searchParams.page}&geo=${searchParams.geo}`,
            {
              headers: {
          ...
          if (!response.ok) {
            ...
            throw new Error(errorData.error || `Amazon search failed with status ${response.status}`);
          }

          const data = await response.json();
          console.log('Edge Function response received for:', searchParams.query);
